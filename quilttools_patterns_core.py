...
def import_block_into_region(parent_tree, parent_region, lib_bd, sizing_mode="stretch", rotation=0.0, flip="none", auto_align=True):
    ...
    def cut_region_by_line(tree, region_id, p1, p2):
        ...
        # Extend the cut line significantly
    ...
    # 2. Partition target region using the outer edges of the mapped library block
    ...
    def copy_subtree(lib_node_id, parent_id_on_canvas):
        ...
        mapped_poly = [map_pt(p) for p in lib_node.polygon]
        ...
        if lib_node.is_leaf():
            ...
            if len(child_ids) == 0:
                ...
                # Collapse redundant intermediate node
